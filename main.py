...
twitter_api = twitter.Api(consumer_key=config.twitter_credentials["consumer_key"],
                          ...
                          access_token_secret=config.twitter_credentials["access_secret"],tweet_mode="extended")
...
#Initialise tweepy to use tag friends functionality
...
def check():
    ...
    searched_tweets = []
    searched_tweets += twitter_api.GetSearch(term="nft giveaway",count='100',result_type="popular")
    searched_tweets += twitter_api.GetSearch(term="nft free",count='100',result_type="popular")

    searched_tweets += twitter_api.GetSearch(term="nft giveaway",count='100',result_type="mixed")
    searched_tweets += twitter_api.GetSearch(term="nft free",count='100',result_type="mixed")

    for i in range(1,5): #20
        searched_tweets += twitter_api.GetSearch(raw_query="q=nft%20giveaway%20-discord%20-instagram%20-telegram%20min_retweets%3A"+str(100*i)+"%20-filter%3Areplies%20-filter%3Aretweets&count=100")
        searched_tweets += twitter_api.GetSearch(raw_query="q=nft%20giveaway%20-discord%20-instagram%20-telegram%20min_retweets%3A"+str(100*i)+"%20filter%3Aretweets&count=100")

    for i in range(1,10):
        searched_tweets += twitter_api.GetSearch(raw_query="q=nft%20giveaway%20-discord%20-instagram%20-telegram%20min_retweets%3A"+str(1000 + 500*i)+"%20-filter%3Areplies%20-filter%3Aretweets&count=100")
        searched_tweets += twitter_api.GetSearch(raw_query="q=nft%20giveaway%20-discord%20-instagram%20-telegram%20min_retweets%3A"+str(1000 + 500*i)+"%20filter%3Aretweets&count=100")

    searched_tweets = list(dict.fromkeys(searched_tweets))
    ...
    for tweet in searched_tweets:
        ...
        for BadWord in config.BadList:
            if (tweet.full_text.lower().find(BadWord) != -1):
                Bad = True
        ...
        if tweet.retweet_count>20 and difference.days<=30:
            if not (Bad): #if tweet doesn't contain bad words
                ...
                if any(x in tweet.full_text.lower().split() for x in config.retweet_tags):
                    # This clause checks if the text contains any retweet_tags
                    if tweet.retweeted_status is not None:
                        # In case it is a retweet, we switch to the original one
                        if any(x in tweet.retweeted_status.full_text.lower().split() for x in config.retweet_tags):
                    ...
                    try:
                        ...
                        try:
                            ...
                            if config.use_msgs is True and any(x in tweet.full_text.lower() for x in config.message_tags):
                        ...
                        #TAG
                        if "tag" in tweet.full_text.lower():
                                    #populate reply string with friend tags
                                    friendsToTag = 3
                                    reply = ""
                                    #for i in range(0,friendsToTag):
                                        #reply += "@" + friends[random.randint(0,len(friends)-1)] + " "

                                    reply = "@Akros333 @Br3ikros @barberis_matteo"

                                    api.update_status(status = reply, in_reply_to_status_id = tweet.id , auto_populate_reply_metadata=True)
                                    replied = True
                                    print("Friends Tagged")
                                    #either delete first line or second line or both
                                    just_retweet_streak += 1
                                    time.sleep(config.retweet_rate)
                        ...
                        if "retweeted" not in str(e):
                            print(colors.FAIL + colors.BOLD + str(e) +"diocane" + colors.ENDC)
                            if(str(e).find("161") != -1):
                                #maximum amount of accounts followed in a day reached
